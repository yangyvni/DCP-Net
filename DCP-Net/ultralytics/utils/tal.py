...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        self.n_max_boxes = gt_bboxes.shape[1] # 每个样本的GT框数量
    ...
    def _forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        Returns:
            ...
            target_scores (torch.Tensor): Target scores with shape (bs, num_total_anchors, num_classes).
            fg_mask (torch.Tensor): Foreground mask with shape (bs, num_total_anchors).
            target_gt_idx (torch.Tensor): Target ground truth indices with shape (bs, num_total_anchors).每个锚框唯一的GT索引（因为一个锚框可能有多个GT分配为正样本，选IoU最大的）
        """
        ...
        # Assigned target(target_score[每个锚框对应的GT的类别=1，其余为0.正样本类别分数为1)
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        align_metric = bbox_scores.pow(self.alpha) * overlaps.pow(self.beta) # 类别分数和iou的加权乘积

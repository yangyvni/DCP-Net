...
from ultralytics.nn.modules import DeformConv
...
class MidBranch(nn.Module):
    ...
    - Input channels = C (output of first DWConv+PW Conv)
    - Two parallel branches:
        * 5x5 depthwise + pointwise
        * 3x3 depthwise + pointwise
      Then element-wise add and return.
    """
    def __init__(self, c: int):
        super().__init__()
        # depthwise(5x5) + pw
        self.conv5 = Conv(c, c, 5)
        self.conv3 = Conv(c, c, 3)
...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        ...
        #     nn.Sequential(Conv(x, c2, 3),  MidBranch(c2), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        # )
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3),  MidBranch(c2),  nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )

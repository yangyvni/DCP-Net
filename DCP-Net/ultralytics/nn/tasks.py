...
from ultralytics.utils.gcls_repul_loss import GCRv8DetectionLoss
from ultralytics.nn.modules.sppf_sfadc import SPPF_SFADC
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            if m.f != -1:  # if not from previous layer
                # print(f"m.f: {m.f}, y_len: {len(y)}")
                x = y[m.f] if isinstance(m.f, int) else [x if j == -1 else y[j] for j in m.f]  # from earlier layers
            if profile:
                self._profile_one_layer(m, x, dt)
            x = m(x)  # run
            if isinstance(x, list):  # 确保输出都是1层一维张张量
                for i in range(len(x)):
                    y.append(x[i])  # save output,若是-1就不在save中
            else:
                if m.i in self.save:   
                    y.append(x) 
                else:
            ...
            # print(f"m.i:{m.i}, m.f: {m.f}, y_len:{len(y)}, self.save: {self.save}")  # print type and index of each layer
            # y.append(x if m.i in self.save else None)  # save output
            if visualize:
    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            for m in self.model.modules():
                if isinstance(m, (Conv, Conv2, DWConv, DeformConv)) and hasattr(m, "bn"):
...
class DetectionModel(BaseModel):
    ...
    def init_criterion(self):
        ...
        return E2EDetectLoss(self) if getattr(self, "end2end", False) else GCRv8DetectionLoss(self)
...
def parse_model(d, ch, verbose=True):
    ...
    offset = 0  # layer index offset
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        real_i = i + offset  # 实际层编号 = offset + 2
        print(f"Processing layer {real_i}: from={f}, n={n}, m={m}, args={args}")
        m = (
        ...
        if m in base_modules:
            ...
            if m in repeat_modules:
                ...
                n = 1
            if m is (C3k2, C3k2SFADC, C3k2DCN):  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is A2C2f:
        ...
        save.extend(x % real_i for x in ([f] if isinstance(f, int) else f) if x != -1)  # append to savelist
        ...
        if isinstance(c2, list):  
            ...
            ch.append(c2)      
        # print(f"!!!!!!save: {save}, f: {f}, i: {real_i}")
        m_.i, m_.f, m_.type = real_i, f, t  # attach index, 'from' index, type
        if verbose:
